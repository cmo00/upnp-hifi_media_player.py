...
def find_dac(debug=False):
    """Auto-detect compatible DAC (soundfile compatible)"""
    try:
        devices = sd.query_devices()
...
def extract_track_number(filepath, debug=False):
    ...
    import re
    # This regex captures leading digits before space, dash, underscore, etc.
    match = re.match(r'^(\d{1,3})\s*[\-_.:\s\.]', basename)  # "01   Title", "7- Song"
...
def list_audio_files(directory, debug=False):
    ...
    try:
        for filename in os.listdir(directory):
            # Skip hidden files (macOS, Linux, etc.)
            ...
            # Store with sort key (tuple for multi-level sorting)
            if track_num is not None:  
                # Files with track numbers:  (0, numeric_track_number, filename)
                # 0 = first priority (files WITH track numbers)
                # numeric_track_number = sort numerically (1, 2, 3, 7, 15, 20, etc.)
                sort_key = (0, track_num, filename)
            else:
                # Files without track numbers: (1, 99999, filename)
                # 1 = second priority (files WITHOUT track numbers)
                # 99999 = dummy value to allow alphabetical sorting by filename
                sort_key = (1, 99999, filename)
            
            entries.append((filename, sort_key))
    
    except Exception as e: 
        print(f"[ERROR] list_audio_files: {e}")
        return []
    
    # Sort entries using the tuple keys
    # Python sorts tuples element by element: 
    # 1. First by first element (0 = with numbers, 1 = without)
    # 2. Then by second element (numeric track number or 99999)
    # 3. Then by third element (filename for tie-breaking)
    ...
    if debug:
        ...
        for i, f in enumerate(sorted_files, 1):
            ...
            tn_str = f"#{tn: 2d}" if tn is not None else "? ?"
            print(f"  {i:2d}.  {tn_str}  {f}")
        print()
    ...
    while True:
        ...
        if choice == 'q':
            ...
            if sel. isdigit():
                ...
                idx, device = find_dac(debug=debug)
                if idx is not None:
                    SHARED_STATE.update({
                        'usb_device_index': idx,
...
%(prog)s --dir /music/ --debug
    ...
    if args.usb and USB_DEVICES_QUERYABLE:
        if usb_device_index is None:
            idx, device = find_dac(debug=args.debug)
            if idx is not None:
                usb_device_index = idx
                usb_device_name = device['name']
